...
class Contig
{
  ...
  friend void write_contig(ostream&, const Contig&,unsigned,bool);
private:
...
public:
  Contig(const Contig & c,unsigned long contig_id) : name(c.name), contig_id(contig_id),set_id(c.set_id), sequence(c.sequence), sequence_size(c.sequence_size), component_contigs(c.component_contigs)
  {}//unused

  Contig(unsigned set_id, unsigned long contig_id, string &&name,vector<Nuc> && sequence) : name(name), contig_id(contig_id), set_id(set_id), sequence(move(sequence)) {
    sequence_size=this->sequence.size();
    component_contigs.emplace_back(sequence_size,name,set_id,0);
  }

  Contig(Contig && c, unsigned set_id, unsigned long contig_id) : name(move(c.name)), contig_id(contig_id),set_id(set_id), sequence(move(c.sequence)), component_contigs(move(c.component_contigs)) {
    //name(move(c.name)), set_id(set_id), sequence(move(c.sequence)), component_contigs(move(c.component_contigs)) {
    sequence_size=this->sequence.size();
  } // unused

    // merge two contigs
  Contig(Contig && c1, Contig && c2, bool is_c1_reversed, bool is_c2_reversed, size_t overlapping_size, unsigned set_id, unsigned long contig_id) :  name("("+ c1.name + "|" + c2.name +")"), contig_id(contig_id), set_id(set_id), sequence_size(c1.size())
  {
    if(!is_c1_reversed){
  ...
  void absorb(Contig &c, unsigned shift, bool reverse)
  {
	...
	if(!(sequence[i+shift]==c.sequence[i])){
	  cout << "coucou\n";
	    exit(EXIT_SUCCESS);
	}
	...
	sequence[i+shift]+=c.sequence[i];
      ...
      for(unsigned i=0; i<c.size(); ++i){
	if(!(sequence[i+shift]==c.sequence[c.sequence_size-1-i].reverse())){
	  cout << "coucou b\n";
	  exit(EXIT_SUCCESS);
	}
...
void write_contig(ostream& os, const Contig& contig,unsigned id, bool random_when_tie)
{
