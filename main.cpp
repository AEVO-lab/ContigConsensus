...
int main(int argc, char *argv[])
{
  ...
  ConsensusConstruction * consensus;
  
  if(options.mode==Options::greedy_general)
    consensus = new Greedy(options.percentage_of_match);
  else consensus = new PairwiseGreedy(options.percentage_of_match);


  consensus->treatFastaDirectory(options.contig_directory.c_str(), false, options.prioritize);

  if(options.generate_matches)
    consensus->createMatchDirectory(options.matches_directory.c_str(), options.contig_directory.c_str());
  ...
  if(options.pairwise){
    ...
    consensus->treatMatchDirectory(options.matches_directory.c_str());
    std::cout << "Consensus construction" << std::endl;
    consensus->merge_algorithm();
    consensus->output_contigs(options.output_fast.c_str(), options.random_when_tie, options.prioritize!="-1");
    consensus->output_contig_ordering(options.output_cons.c_str(), options.prioritize!="-1");
  }
  // for(auto &p : data.ids){
